...
.ThenInclude(aq => aq.Answers)
    ...
    // Auto-end the proctor session when attempt is submitted
...
AnsweredAt = a.AnsweredAt
  ...
  public async Task<ApiResponse<List<AttemptEventDto>>> GetAttemptEventsAsync(int attemptId)
  {
    ...
    // Enrich AnswerSaved events with question text and answer summary
    var answerSavedEvents = events.Where(e => e.EventType == AttemptEventType.AnswerSaved).ToList();
    if (answerSavedEvents.Any())
    {
      // Extract questionIds from metadata
      var questionIds = new HashSet<int>();
      foreach (var evt in answerSavedEvents)
      {
        if (!string.IsNullOrEmpty(evt.MetadataJson))
        {
          try
          {
            var meta = JsonSerializer.Deserialize<JsonElement>(evt.MetadataJson);
            if (meta.TryGetProperty("questionId", out var qIdProp))
              questionIds.Add(qIdProp.GetInt32());
          }
          catch { }
        }
      }
      ...
      if (questionIds.Any())
      {
        // Load questions with options and answers in one query
        ...
        var questionLookup = attemptQuestions.ToDictionary(aq => aq.QuestionId);

        foreach (var evt in answerSavedEvents)
        {
          if (string.IsNullOrEmpty(evt.MetadataJson)) continue;
          try
          {
            var meta = JsonSerializer.Deserialize<JsonElement>(evt.MetadataJson);
            if (!meta.TryGetProperty("questionId", out var qIdProp)) continue;
            var qId = qIdProp.GetInt32();

            if (questionLookup.TryGetValue(qId, out var aq))
            {
              evt.QuestionTextEn = aq.Question.BodyEn;
              evt.QuestionTextAr = aq.Question.BodyAr;

              // Build answer summary from the latest answer
              var answer = aq.Answers.OrderByDescending(a => a.AnsweredAt).FirstOrDefault();
              if (answer != null)
              {
                if (!string.IsNullOrEmpty(answer.SelectedOptionIdsJson))
                {
                  ...
                  {
                    var selectedOptions = aq.Question.Options
                        .Where(o => selectedIds.Contains(o.Id))
                        .OrderBy(o => o.Order)
                        .ToList();
                    evt.AnswerSummary = string.Join(", ", selectedOptions.Select(o => o.TextEn));
                  }
                }
                else if (!string.IsNullOrEmpty(answer.TextAnswer))
                {
                  evt.AnswerSummary = answer.TextAnswer.Length > 100
