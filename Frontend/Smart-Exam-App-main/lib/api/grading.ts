...
export async function getGradingSessions(params?: {
  ...
  const inner = raw && typeof raw === "object"
    ...
    : {}
  const items = (inner?.items ?? inner?.Items ?? []) as GradingSessionListItem[]
  return {
    ...
    hasPreviousPage: (inner?.hasPreviousPage ?? inner?.HasPreviousPage ?? false) as boolean,
 ...
 */
export async function getGradingSessionByAttempt(attemptId: number): Promise<GradingSessionDetail | null> {
  try {
    return await apiClient.get<GradingSessionDetail>(`/Grading/attempt/${attemptId}`)
  } catch {
    return null
  }
}
...
export async function getManualGradingQueue(gradingSessionId: number): Promise<GradedAnswerItem[]> {
  const response = await apiClient.get<GradedAnswerItem[]>(`/Grading/${gradingSessionId}/manual-queue`)
  return response || []
}
 ...
 */
export async function submitManualGrade(grade: ManualGradeRequest): Promise<void> {
...
export async function completeGrading(gradingSessionId: number): Promise<{
  ...
  // Finalize result so candidate can see it (backend also does this on Complete; this is fallback)
 ...
 */
export async function finalizeResult(gradingSessionId: number): Promise<boolean> {
  try {
    await apiClient.post(`/ExamResult/finalize/${gradingSessionId}`)
    return true
  } catch (err) {
