...
export async function getCandidateExamDetails(params: {
  ...
  if (params.screenshotLimit)
    query.set("screenshotLimit", params.screenshotLimit.toString());

  return await apiClient.get<CandidateExamDetailsDto>(
    `/candidate-exam-details?${query}`,
  );
}

export async function getCandidateExams(
  candidateId: string,
): Promise<CandidateExamBriefDto[]> {
