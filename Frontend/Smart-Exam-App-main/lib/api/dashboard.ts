...
export async function getDashboardStats(): Promise<DashboardStats> {
  ...
  try {
    const [examsRes, gradingRes, incidentRes] = await Promise.all([
      getExams({ pageNumber: 1, pageSize: 1 }).catch(() => ({
        totalCount: 0,
        items: [],
      })),
      getManualGradingRequired({ pageNumber: 1, pageSize: 1 }).catch(() => ({
        items: [],
        totalCount: 0,
      })),
      apiClient
    ...
    if (incidentRes && typeof incidentRes === "object") {
      openIncidents =
        (incidentRes as { openCases?: number }).openCases ??
        (incidentRes as { totalCases?: number }).totalCases ??
        0;
    }
  } catch {
    // use defaults
